...
const inflate = async (lrcBuf: Buffer) =>
...
export default () => {
  mainHandle<
    { lrc: string; tlrc: string; rlrc: string },
    { lyric: string; tlyric: string; rlyric: string }
  >(
    WIN_MAIN_RENDERER_EVENT_NAME.handle_tx_decode_lyric,
    async ({ params: { lrc, tlrc, rlrc } }) => {
